...
funcs.json = require('Communication/MQTTClient/helper/Json')
-- Default parameters for instances of module
funcs.defaultParameters = require('Communication/MQTTClient/MQTTClient_Parameters')
...
local function createJsonListSubscriptions(content)
  local orderedTable = {}
  ...
  if content == nil then
    ...
    for _, value in pairs(orderedTable) do
      table.insert(subscriptionList, {DTC_SubTopic = value, DTC_SubQos = content[value]})
    end
...
--@createJsonListPublishEvents(content:table)
...
---@return string jsonstring JSON string
local function createJsonListPublishEvents(content)
  local orderedTable = {}
  ...
  if content.topic == nil then
    ...
    for _, value in pairs(orderedTable) do
      table.insert(subscriptionList, {DTC_Event = value, DTC_PublishTopic = content.topic[value], DTC_PublishQos = content.qos[value], DTC_PublishRetain = content.retain[value],})
    end
